...
Description:
This module contains the job submission service, which is responsible for submitting jobs to the on-prem cluster and cloud cluster via the `submission_loop` method. Jobs are submitted to cluster based on their arrival times. When all jobs have completed, the service exits.
"""
import logging
import os
import subprocess
...
class JobStateTracker(object):
	"""
	Tracks the job submission and completion over the cloud and on-prem cluster.
	"""
	...
	def update_submit_file_state(self, idx: int, file_path: str):
		"""
		Updates the submission state of the job with the given index to a given file.
		"""
	...
	def update_finished_jobs(self, onprem_cluster: Dict[str, Any], cloud_cluster: Dict[str, Any]):
		"""
		Updates the finished state of the jobs. For the on-premise cluster, it polls Kubernetes for the job status. For the cloud-cluster, depending on the spillover configuration, it either polls the cloud cluster or parses the log file.
		...
		for idx, cluster_config in enumerate(configs): 
			...
			job_list, _, _ = api.list_namespaced_job_with_http_info(namespace="default", limit=None, _continue='')		
			for job_idx, job_state in self.finish_state.items(): 
				try:
					if not job_state:
						curr_job = find_job_with_substring(job_list.items, "job-" + str(job_idx))
...
def get_job_submission_log_path(name: str):
	...
	"""
	path =  f'{sweep_logger.LOG_DIRECTORY.format(name=name)}/debug/job_submission_thread.log'
...
def get_event_submission_log_path(name: str, idx:int):
	...
	"""
	path =   f'{sweep_logger.LOG_DIRECTORY.format(name=str(name))}/events/{idx}.log'
	return os.path.abspath(path)
...
def submit_job_to_scheduler(job_yaml: dict):
    with tempfile.NamedTemporaryFile(mode='w') as f:
        ...
        logger.debug(f'****** Job Sent to submit_job.py at Time {time.time()} ******')
        subprocess.run(['python3', '-m', 'starburst.drivers.submit_job', '--job-yaml', temp_file_path])

def job_submission_service(jobs: Dict[Any, Any], clusters: Dict[str, str], sweep_name: str, run_index: int):
	"""
	Calls loop that submit jobs to the scheduler. If an error occurs during the submission loop, the error is logged and the next run is started.
	"""
	submission_file_logger = sweep_logger.LogFileManager('job_submission_service', get_job_submission_log_path(sweep_name))
	try:
		submission_loop(jobs=jobs, clusters=clusters, sweep_name=sweep_name, run_index=run_index, file_logger=submission_file_logger)
	except Exception as e:
		# If an error occrus during job submission loop, log the error and move onto the next run in the sweep.
		submission_file_logger.append(traceback.format_exc() + '\n')
		pass
	submission_file_logger.close()

def submission_loop(jobs: Dict[Any, Any], clusters: Dict[str, str], sweep_name: str, run_index: int, file_logger: sweep_logger.LogFileManager):
	"""
	Logic for submitting jobs to the scheduler. Only returns when all jobs have been submitted and completed running on the cluster.
	"""
	total_jobs = len(jobs)
	...
	cloud_cluster = {
		...
		'cloud_cluster_path': get_event_submission_log_path(sweep_name, run_index),
	}
	
	# Primary loop for submitting jobs to the scheduler. Will submit jobs until all jobs have been submitted.
	...
	while True:
		...
		if time.time() > jobs[job_index]['submit_time'] + submission_offset:
			...
			job_tracker.update_submit_file_state(job_index, \
					 f'{sweep_logger.LOG_DIRECTORY.format(name=str(sweep_name))}/jobs/{run_index}.yaml')
			file_logger.append(f'Submitting Job {job_index}: {str(job)} at Time {submit_time}.')
			job_index += 1
		if job_index >= total_jobs: 
			break
		time.sleep(JOB_SUBMISSION_TICK)
	assert job_tracker.check_if_jobs_submitted(), "Not all jobs were submitted to the scheduler."

	# Once all jobs have been submitted, continually check the status of the jobs until all jobs have completed.
	...
	while True:
		...
		file_logger.append("Submitted Jobs State: " + str(job_tracker.finish_state))
