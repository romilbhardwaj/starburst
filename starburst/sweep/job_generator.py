...
class JobGenerator(object):
	...
	The job generator is responsible for generating job resources, interarrival times, and durations.
	It also tackels 
	'''
	...
	def get_jinja_dict(self, job:dict) -> dict:
		...
		Args:
			job: A dictionary containing the job_id, job_duration, and resources.
		"""
		return NotImplementedError
	...
	def script(self) -> str:
		"""
		Returns the running script to use for this job. Can be a template script.
		"""
		return NotImplementedError
...
class GPUTrainJobGenerator(JobGenerator):
	"""
	A job generator that generates GPU training jobs. See training_dataset.py for more details.
	"""
	def __init__(self, config) -> None:
	...
	def get_duration(self):
		...
		estimated_duration, script = utils.sample_gpu_train_job(self.cached_values['gpu'], generated_duration)
...
def generate_jobs(run_config: utils.RunConfig):
	...
	Args:
		run_config: The run configuration object, which specifies the hyperparameters for this run.
	"""
	rc = run_config
	...
	job_gen = job_gen_class({key: all_params[key] for key in JOB_GEN_ARRAY if key in all_params})
